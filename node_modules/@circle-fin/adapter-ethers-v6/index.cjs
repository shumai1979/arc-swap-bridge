...
function getDefaultProviderUtil(cache) {
    return ({ chain }) => {
        ...
        try {
            const newJsonRpcProvider = new ethers.JsonRpcProvider(rpcUrl);
...
async function switchChain(signer, chain) {
    ...
    const rpcUrl = chain.rpcEndpoints[0];
    const newProvider = new ethers.JsonRpcProvider(rpcUrl, chain.chainId);
    if (signer instanceof ethers.Wallet && signer.privateKey) {
        const privateKey = signer.privateKey;
        if (typeof privateKey === 'string') {
            return new ethers.Wallet(privateKey, newProvider);
        }
    }
...
const ethersAdapterOptionsSchema = zod.z.object({
    getProvider: zod.z
        .function()
